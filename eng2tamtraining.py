...
with open('tamil.txt','r',encoding='utf-8') as f:
    rows=f.read().split('\n')
#read first 10 rows from dataset     
for row in rows[:10]:
    # Check if the row contains a tab character
    if '\t' in row:
        input_text, target_text = row.split('\t')
        # Add '\t' at start and '\n' at end of target text.
        target_text = '\t' + target_text + '\n'
        input_texts.append(input_text.lower())
        target_texts.append(target_text.lower())
        # Split characters from text and add them to the respective sets
        input_characters.update(list(input_text.lower()))
        target_characters.update(list(target_text.lower()))
    else:
        # Handle the case where the row does not contain a tab character
...
pickle.dump({'input_characters':input_characters,'target_characters':target_characters,
             ...
             'num_en_chars':num_en_chars,'num_dec_chars':num_dec_chars},open("training_data_tamil.pkl","wb"))
